		...
		<div class="card card-outline card-primary">
			<div class="card-header">
				...
				<div class="card-tools">
					<a href="{{url('/admin/ventas/create')}}" class="btn btn-primary"><i class="fa fa-plus"></i> Crear
						nuevo</a>
				</div>
			</div>
			<!-- /.card-header -->
					...
					<tbody>
						...
						@foreach ($ventas as $venta)
							<tr>
                                ...
                                <td>{{ $venta->cliente->nombre_cliente ?? 'Sin nombre' }}</td>
								<td>$ {{ $venta->precio_total ?? 'Sin precio total' }}</td>
								<td style="text-align: center;">
									<div class="btn-group" role="group" aria-label="Basic example">
										<a href="{{ url('/admin/ventas', $venta->id) }}" class="btn btn-info"><i
												class="fas fa-eye"></i></a>
										<a href="{{ url('/admin/ventas/' . $venta->id . '/edit') }}" type="button"
											class="btn btn-success"><i class="fas fa-pencil"></i></a>
										<a href="{{ url('/admin/ventas/pdf/' . $venta->id) }}" type="button"
	...
	<script>
		$('#mitabla').DataTable({
			...
			"language":{
				...
				"infoEmpty": "Mostrando de 0 a 0 de 0 ventas",
